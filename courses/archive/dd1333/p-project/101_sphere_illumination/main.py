...
from lib.linalg import Plane, Vector3d
from lib.mathassets import Shadow, generate_sphere
...
def stringify_sphere(
    sphere: list[list[float | str | None]], symbol_set: dict[str, str]
) -> str:
    sphere_string: str = ""
    for row in sphere:
        for e in row:
            ...
            if e is None:
                ...
                symbol = symbol_set["shadow"]
            elif float(e) <= 0:
                symbol = symbol_set["no_light"]
            elif 0 < float(e) and float(e) <= 0.3:
                symbol = symbol_set["dark"]
            elif 0.3 < float(e) and float(e) <= 0.5:
                symbol = symbol_set["slightly_dark"]
            elif 0.5 < float(e) and float(e) <= 0.7:
                symbol = symbol_set["slightly_bright"]
            elif 0.7 < float(e) and float(e) <= 0.9:
...
def input_float(name: str) -> float:
    while True:
        try:
            ...
            print_error("Invalid input", '"." Must be used for the decimal point.')
            continue
...
#
# outputs: no output
def change_light_source(root: tk.Tk, event: tk.Event, **kwargs: Any):
    ...
    font: tkf.Font = tkf.Font(
        family=SPHERE_FONT["family"],
        size=SPHERE_FONT["size"],
        weight=SPHERE_FONT["weight"],
    )
    ...
    if discriminant < 0:
        instruction_label.configure(
            text="That is not a point in the sphere! Try another point", fg="red"
        )
        return None
    ...
    sphere = generate_sphere(radius, light_source, ZOOM)
...
def display_sphere(sphere_string: str, sphere: list[list[Any]], radius: float):
    ...
    label = tk.Label(
        root, text=sphere_string, font=(SPHERE_FONT["family"], SPHERE_FONT["size"])
    )
    label.pack()
    ...
    label.bind(
        "<Button-1>",
        lambda event: change_light_source(
            root,
            event,
            radius=radius,
            label=label,
...
def main():
    ...
    discriminant: float = radius**2 - x_0**2 - y_0**2
    # If the discriminant is zero, there is a divison by zero error in the
    # generate_sphere function
    while discriminant <= 0:
        print_error(
            "Light source point not inside the sphere!",
            "Use a pair of x0 and y0 which exist inside the sphere. Boundary points are also invalid",
        )
    ...
    try:
        ...
        print("Encountered a division by zero when generating the sphere.")
        print("Exiting...")
        return
    ...
    while file_output not in ["Y", "N"]:
        print(Fore.RED + "Not an option." + Fore.RESET)
        file_output = (
            input("Would you like to have this written to a file [Y/n]? ")
            .upper()
            .strip()
        )
    ...
    if output_file:
        default_filename: str = (
            datetime.today().strftime("%Y-%m-%d_%H:%M") + "_sphere.txt"
        )
        ...
        while different_file_name not in ["Y", "N"]:
            print(Fore.RED + "Not an option." + Fore.RESET)
            different_file_name = (
                input(
                    f"The default filename will be {default_filename}, would you like to call it something else [Y/n]? "
